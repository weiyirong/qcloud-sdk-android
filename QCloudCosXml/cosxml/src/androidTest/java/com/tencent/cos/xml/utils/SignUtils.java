...
public class SignUtils {
    ...
    public void setQHeaderList(Map<String, Object> headerMap) {
        if (headerMap != null) {
            ...
            for (Map.Entry<String, Object> entry : headerMap.entrySet()) {
                temp.put(entry.getKey().toLowerCase().trim(), StringUtil.encodedUrl(((String) entry.getValue()).trim()));
            }
    ...
    public void setQUrlParamList(Map<String, Object> urlParamList) {
        if (urlParamList != null) {
            ...
            for (Map.Entry<String, Object> entry : urlParamList.entrySet()) {
                temp.put(StringUtil.encodedUrl(entry.getKey().trim()).toLowerCase(),
                        StringUtil.encodedUrl(((String) entry.getValue()).trim()).toLowerCase());
            }
    ...
    protected String getSignkey(String secretKey, String qKeyTime) {
        ...
        try {
            ...
            mac.init(hmacKey);
            signKey = StringUtil.toHexString(mac.doFinal(qKeyTime.getBytes("utf-8")));
    ...
    protected String getSignature(String signKey, String stringToSign) {
        ...
        try {
            ...
            Mac mac = Mac.getInstance("HmacSHA1");
            mac.init(hmacKey);
            signature = StringUtil.toHexString(mac.doFinal(stringToSign.getBytes("utf-8")));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
